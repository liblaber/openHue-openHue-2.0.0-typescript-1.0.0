...
import { z } from 'zod';
import { supportedTimedEffects } from '../../common/supported-timed-effects';
 ...
 */
export const lightPutTimedEffectsResponse = z
  .object({
    effect: supportedTimedEffects.optional(),
    duration: z.number().optional(),
  })
 ...
 * Is equal to application shape if all property names match the api schema
 */
export const lightPutTimedEffectsRequest = z
  .object({ effect: supportedTimedEffects.nullish(), duration: z.number().nullish() })
  .transform((data) => ({
    effect: data['effect'],
    duration: data['duration'],
  }));
