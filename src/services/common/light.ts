...
import { z } from 'zod';
import { lightLevelReport, lightLevelReportRequest, lightLevelReportResponse } from './light-level-report';
...
export const lightResponse = z
  .object({
    ...
    light_level_report: lightLevelReportResponse.optional(),
  })
  .transform((data) => ({
 ...
 * Is equal to application shape if all property names match the api schema
 */
export const lightRequest = z
  .object({
    lightLevel: z.number().nullish(),
    lightLevelValid: z.boolean().nullish(),
    lightLevelReport: lightLevelReportRequest.nullish(),
  })
  .transform((data) => ({
